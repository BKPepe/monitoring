          ...
          <Card className="w-full max-w-2xl p-6 relative space-y-4 max-h-[85vh] overflow-y-auto bg-slate-900 border-slate-700 text-slate-100 shadow-2xl">
            ...
            <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{t('footer.docs_subtitle', 'Příručka k monitorování, API a instalačním agentům.')}</p>
              </div>
            </div>

            <div className="space-y-4 text-xs text-slate-300">
              <section className="space-y-1.5">
                <h4 className="font-semibold text-white text-sm">{t('footer.docs_section1_title', '1. Monitoring HTTP & SSL Webů')}</h4>
                <p className="leading-relaxed">
                  {t('footer.docs_section1_desc', 'Systém pravidelně v 60s intervalu testuje dostupnost vašich webů, vyhodnocuje latenci odpovědi, HTTP stavové kódy a platnost SSL/TLS certifikátů.')}
                </p>
              </section>

              <section className="space-y-1.5">
                <h4 className="font-semibold text-white text-sm">{t('footer.docs_section2_title', '2. Instalace Systémového Agenta')}</h4>
                <p className="leading-relaxed">
                  {t('footer.docs_section2_desc_prefix', 'Pro měření CPU, RAM a zaplnění diskových oddílů na Linux/OpenWrt serverech použijte jednorázový instalační skript v sekci')} <a href="/app/api-agents" className="text-sky-400 underline">{t('nav.api-agents', 'API & Agenti')}</a>.
                </p>
              </section>

              <section className="space-y-1.5">
                <h4 className="font-semibold text-white text-sm">{t('footer.docs_section3_title', '3. Veřejné API & Prometheus Exportér')}</h4>
          ...
          <Card className="w-full max-w-md p-6 relative space-y-4 bg-slate-900 border-slate-700 text-slate-100 shadow-2xl">
            ...
            <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">{t('footer.support_subtitle', 'Podpora pro monitorovací systém Blood Kings.')}</p>
              </div>
            </div>
