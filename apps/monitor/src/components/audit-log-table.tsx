    ...
    <Card className="p-6 space-y-4 border-primary/25">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-base">{t('audit_log.title', 'Systémový Auditní Protokol & Logy Aktivitu')}</h3>
            <p className="text-xs text-muted-foreground">{t('audit_log.subtitle', 'Živý záznam bezpečnostních událostí, přihlášení a automatických kontrol')}</p>
          </div>
        </div>
                ...
                <tr key={row.id} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="py-3 px-3 whitespace-nowrap">
                    <Badge variant={row.status === 'down' ? 'down' : 'up'} className="font-bold">
                      {row.status === 'down' ? t('audit_log.status_error', 'CHYBA') : 'OK'}
                    </Badge>
                  </td>
