...
function buildGenericAsset(id: number): AssetDetail {
  return {
    ...
    name: `Zařízení #${id}`,
    kind: 'MONITOR',
    subtitle: `Zařízení #${id} · Načítám z databáze...`,
    status: 'up',
    breadcrumb: ['Infrastructure', 'Monitory'],
    health: [
      { key: 'status', label: 'Stav', value: 'Online' },
      { key: 'latency', label: 'Odezva', value: '—', tone: 'latency' },
    ...
    summary: `Diagnostické metriky zařízení #${id} načtené z databáze.`,
    summaryChips: [{ label: 'Synchronizuji s API', variant: 'info' }],
    info: [{ label: 'ID Monitoru', value: String(id) }],
    smartStatus: null,
    events: [],
    processes: [],
    related: [],
