...
export function SetupPage() {
  ...
  // Forgotten password modal state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(installed ? t('setup.login_success', 'Přihlášení úspěšné! Přesměrovávám na dashboard...') : t('setup.install_success', 'Instalace úspěšná! Přesměrovávám...'));
      setTimeout(() => {
      ...
      if (err.message && err.message.includes('2FA')) {
        setRequire2FA(true);
        setError(t('setup.totp_required_error', 'Účet má aktivní 2FA. Zadejte 6-místný kód z autentikační aplikace (Google/Microsoft Authenticator) a klikněte na Přihlásit znovu.'));
      } else {
        setError(err.message || t('setup.login_failed', 'Přihlášení selhalo. Zkontrolujte své přihlašovací údaje.'));
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        setForgotMsg(t('setup.forgot_processed', { email: forgotEmail }, `Žádost o reset hesla pro ${forgotEmail} byla zpracována. Pokud účet existuje, obdržíte e-mail s instrukcemi.`));
      }
    } catch {
      setForgotMsg(t('setup.forgot_sent_to', { email: forgotEmail }, `Na e-mail ${forgotEmail} byly odeslány instrukce pro obnovu hesla.`));
    } finally {
  ...
  async function tryLogin(u: string, p: string, totp?: string) {
    ...
    if (res.ok && data.requires2fa) {
      throw new Error(t('setup.totp_required', 'Vyžadováno dvoufázové ověření (2FA).'));
    }
    if (res.ok && data.success) {
      return;
    }
    throw new Error(data.message || t('setup.login_failed', 'Přihlášení selhalo. Zkontrolujte své přihlašovací údaje.'));
  }
  ...
  async function tryInstall(u: string, e: string, p: string) {
    ...
    if (!res.ok) {
      if (text.startsWith('{')) {
        ...
        throw new Error(json.message || json.error || t('setup.install_failed', 'Instalace selhala.'));
      }
      throw new Error(t('setup.install_failed', 'Instalace selhala.'));
    }
  }
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0f172a', color: '#f8fafc', fontFamily: 'sans-serif', padding: '1rem' }}>
      <div style={{ width: '100%', maxWidth: '440px', padding: '2rem', background: '#1e293b', borderRadius: '1rem', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.5)', border: '1px solid #334155' }}>
        ...
        <p style={{ color: '#94a3b8', fontSize: '0.875rem', marginBottom: '1.5rem' }}>
          {installed ? t('setup.login_subtitle', 'Přihlášení správce do monitorovacího systému') : t('setup.install_subtitle', 'Vítejte v instalaci monitorovacího rozhraní. Vytvořte první administrátorský účet.')}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {t('users.field_username', 'Uživatelské jméno')}
            </label>
            <input
              type="text"
              required
          ...
          {!installed && (
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#cbd5e1', marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {t('setup.admin_email_label', 'E-mail administrátora')}
              </label>
              <input
                type="email"
                required
          ...
          {(require2FA || totpCode) && (
            <div style={{ background: 'rgba(56, 189, 248, 0.1)', border: '1px solid #38bdf8', padding: '1rem', borderRadius: '0.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#38bdf8', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                🔑 {t('setup.totp_label', 'Ověřovací 2FA Kód (Google / Microsoft Authenticator)')}
              </label>
              <input
                type="text"
                autoFocus
              ...
              <p style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.375rem' }}>
                {t('setup.totp_hint', 'Zadejte aktuální 6místný kód vygenerovaný v mobilní aplikaci.')}
              </p>
            </div>
          )}
        ...
        {installed && (
          <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid #334155' }}>
            ...
            >
              ...
              {t('setup.oauth_github', 'Přihlásit se přes GitHub OAuth SSO')}
            </a>
          </div>
        )}
        ...
        {showForgot && (
          <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #334155' }}>
            <h3 style={{ fontSize: '0.875rem', fontWeight: 600, color: '#f8fafc', marginBottom: '0.5rem' }}>{t('setup.forgot_title', 'Obnovení zapomenutého hesla')}</h3>
            {forgotMsg ? (
              ...
              <form onSubmit={handleForgotSubmit} style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                >
                  {forgotLoading ? '...' : t('setup.forgot_submit', 'Odeslat')}
                </button>
              </form>
            )}
          </div>
