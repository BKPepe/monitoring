...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_monitor']) && $user_role === 'admin') {
    ...
    // Heartbeat: interval je povinný (bez něj není podle čeho poznat zpoždění),
    // tolerance volitelná. Formulář posílá minuty, do databáze jdou sekundy.
    $heartbeat_interval = ($type === 'heartbeat' && !empty($_POST['heartbeat_interval_mins']))
        ? max(1, intval($_POST['heartbeat_interval_mins'])) * 60
        : null;
    $heartbeat_grace = ($type === 'heartbeat' && isset($_POST['heartbeat_grace_mins']) && $_POST['heartbeat_grace_mins'] !== '')
        ? max(0, intval($_POST['heartbeat_grace_mins'])) * 60
        : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_settings']) && $user_role === 'admin') {
    bk_csrf_check();
    $settings_to_save = bk_settings_keys();
    ...
    try {
        ...
        foreach ($settings_to_save as $key) {
            ...
            if (in_array($key, $checkbox_settings, true)) {
                ...
                // A key this form does not have at all (React knows more of them)
                // is skipped. It used to write an empty string, so saving the
                // admin silently erased settings it does not know about - exactly
                // the kind of data loss nobody causes and nobody sees. A field
                // cleared by the user is unaffected: the browser does send an
                // empty string, so `isset` holds.
                continue;
            }
            // Do protokolu patří jen to, co se opravdu změnilo. Dřív se
            // zapisoval seznam všech polí formuláře, takže záznam tvrdil, že
            // se změnilo čtyřicet nastavení, i když šlo o jedno - a k ničemu
            // to nebylo, protože se z něj nedalo poznat co.
            $old_val = $system_settings[$key] ?? null;
            if ($old_val !== $val) {
        ...
        // Hodnoty se do protokolu nepíšou: mezi nastaveními jsou hesla SMTP,
        // tokeny a webhooky. Stačí, které klíče se dotkly.
        if (!empty($changed_settings)) {
            bk_audit_log($pdo, 'settings_updated', implode(', ', $changed_settings));
        }
...
/etc/init.d/cron restart</pre>
                ...
                <div class="admin-card">
                    ...
                    <form action="admin.php" method="POST" id="settings-form">
                        ...
                        <div class="form-group">
                            <label for="trusted_proxies">Důvěryhodné proxy (CIDR, oddělené čárkou)</label>
                            <input type="text" name="trusted_proxies" id="trusted_proxies" value="<?php echo htmlspecialchars(get_setting('trusted_proxies', '')); ?>" class="form-control" placeholder="např. 10.0.0.0/8, 2001:db8::/32">
                            <small style="font-size: 0.75rem; color: var(--text-muted);">
                                Jen pro vlastní reverzní proxy (nginx, HAProxy). Rozsahy Cloudflare jsou zabudované, ty sem psát netřeba.
                                Z těchto adres se věří hlavičce s IP návštěvníka - odkudkoli jinam by si každý mohl do protokolu zapsat cizí adresu a obejít zamykání účtu.
                                Prázdné = důvěřuje se jen Cloudflare.
                            </small>
                        </div>
                        <div class="form-group">
                            <label for="collection_max_age_secs">Hlídač sběru dat: limit stáří (sekundy)</label>
                            <input type="number" min="60" name="collection_max_age_secs" id="collection_max_age_secs" value="<?php echo htmlspecialchars(get_setting('collection_max_age_secs', '900')); ?>" class="form-control" placeholder="900">
                            <small style="font-size: 0.75rem; color: var(--text-muted);">
                                Když poslední dokončený běh cronu zestárne nad tuhle hodnotu, ohlásí to hlídač běžící mimo tento server (viz <code>api.php?action=collection_health</code>). Výchozí 900 s = 15 minut.
                            </small>
                        </div>
                        <!-- Eskalace: pojistka pro upozornění, které nikdo neviděl. -->
                        <div class="form-group">
                            <label for="escalation_enabled">Eskalace nepřevzatých výpadků</label>
                            <select name="escalation_enabled" id="escalation_enabled" class="form-control">
                                <option value="0" <?php echo get_setting('escalation_enabled', '0') !== '1' ? 'selected' : ''; ?>>Vypnuto</option>
                                <option value="1" <?php echo get_setting('escalation_enabled', '0') === '1' ? 'selected' : ''; ?>>Zapnuto</option>
                            </select>
                            <small style="font-size: 0.75rem; color: var(--text-muted);">
                                Když výpadek nikdo nepřevezme (tlačítko Acknowledge u incidentu) do nastavené doby, ohlásí se ještě jednou na jiný kanál. Každý incident eskaluje nejvýš jednou.
                            </small>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="escalation_after_mins">Lhůta na převzetí (minuty)</label>
                                <input type="number" min="1" name="escalation_after_mins" id="escalation_after_mins" value="<?php echo htmlspecialchars(get_setting('escalation_after_mins', '15')); ?>" class="form-control" placeholder="15">
                            </div>
                            <div class="form-group">
                                <label for="escalation_webhook_url">Eskalační webhook (Discord/Slack)</label>
                                <input type="url" name="escalation_webhook_url" id="escalation_webhook_url" value="<?php echo htmlspecialchars(get_setting('escalation_webhook_url')); ?>" class="form-control" placeholder="https://discord.com/api/webhooks/...">
                                <small style="font-size: 0.75rem; color: var(--text-muted);">
                                    Záměrně jiný kanál než běžná upozornění - eskalace má smysl tam, kde první zpráva zapadla. Bez vyplnění se eskalace neodesílá a incidenty na ni čekají dál.
                                </small>
                            </div>
                        </div>
                        <div class="form-group">
                    ...
                    <form action="admin.php" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" id="type" class="form-control" onchange="selectProfileType(this.value);">
                                ...
                                <option value="heartbeat" <?php echo ($edit_monitor && $edit_monitor['type'] === 'heartbeat') ? 'selected' : ''; ?>>Heartbeat (úloha se hlásí sama)</option>
                            </select>
                        ...
                        <div class="form-group" id="heartbeat-group" style="display: <?php echo ($edit_monitor && $edit_monitor['type'] === 'heartbeat') ? 'block' : 'none'; ?>;">
                            <label for="heartbeat_interval_mins">Jak často se úloha ozve (minuty)</label>
                            <input type="number" min="1" name="heartbeat_interval_mins" id="heartbeat_interval_mins" value="<?php echo ($edit_monitor && !empty($edit_monitor['heartbeat_interval'])) ? (int)round($edit_monitor['heartbeat_interval'] / 60) : 60; ?>" class="form-control" placeholder="60">
                            <small style="font-size: 0.75rem; color: var(--text-muted);">Když se úloha neozve do tohoto času (plus tolerance níže), monitor spadne do výpadku.</small>

                            <label for="heartbeat_grace_mins" style="margin-top: 0.75rem; display: block;">Tolerance navíc (minuty)</label>
                            <input type="number" min="0" name="heartbeat_grace_mins" id="heartbeat_grace_mins" value="<?php echo ($edit_monitor && $edit_monitor['heartbeat_grace'] !== null) ? (int)round($edit_monitor['heartbeat_grace'] / 60) : 5; ?>" class="form-control" placeholder="5">
                            <small style="font-size: 0.75rem; color: var(--text-muted);">Záloha nedoběhne vždy na sekundu stejně - tolerance brání planým poplachům.</small>

<?php if ($edit_monitor && $edit_monitor['type'] === 'heartbeat'):
    // Token vzniká při vytvoření; u monitorů založených dřív, než tenhle typ
    // existoval, chybí - tady se doplní, ať administrátor nekouká na prázdno.
    $hb_token = $edit_monitor['heartbeat_token'] ?? '';
    if ($hb_token === '') {
        $hb_token = bk_heartbeat_generate_token();
        $stmt_hb_tok = $pdo->prepare("UPDATE monitors SET heartbeat_token = ? WHERE id = ?");
        $stmt_hb_tok->execute([$hb_token, $edit_monitor['id']]);
    }
    $hb_scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $hb_url = $hb_scheme . '://' . ($_SERVER['HTTP_HOST'] ?? '') . rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? '/status/admin.php'), '/') . '/heartbeat.php?token=' . $hb_token;
?>
                            <div style="margin-top: 1rem; padding: 0.75rem; border: 1px solid var(--border); border-radius: 6px;">
                                <strong style="font-size: 0.85rem;">Adresa, na kterou se úloha hlásí</strong>
                                <p style="margin: 0.35rem 0 0; font-family: monospace; font-size: 0.75rem; word-break: break-all;"><?php echo htmlspecialchars($hb_url); ?></p>
                                <p style="margin: 0.5rem 0 0; font-size: 0.75rem; color: var(--text-muted);">
                                    Na konec úlohy přidejte:<br>
                                    <code style="font-size: 0.72rem;">curl -fsS -m 10 "<?php echo htmlspecialchars($hb_url); ?>"</code><br>
                                    Při selhání doplňte <code>&amp;status=fail&amp;msg=popis</code> - monitor pak spadne hned, ne až po vypršení intervalu.
                                </p>
                            </div>
<?php endif; ?>
                        </div>
    ...
    <script>
        ...
        function togglePortField(type) {
            ...
            const heartbeatGroup = document.getElementById('heartbeat-group');
            const targetGroup = targetInput ? targetInput.closest('.form-group') : null;
            ...
            // Heartbeat nemá cíl - úloha se hlásí sama, není kam se připojovat.
            if (targetGroup) {
            ...
            if (type === 'heartbeat') {
                ...
                if (processesGroup) processesGroup.style.display = 'none';
                if (agentThresholdsGroup) agentThresholdsGroup.style.display = 'none';
            } else if (type === 'web') {
                if (cpanelStatsGroup) {
