...
// Executive summary
$exec_summary = bk_build_executive_summary($monitor, ['score' => $health_score], [], $monitor_insights, array_slice($timeline, 0, 3), $pdo, is_array($details) ? $details : []);
...
(function () {
    ...
    function loadChart(cfg, period) {
        ...
        if (cfg.metrics[0] === '__response__') {
            // Dřív se volalo action=response_history, které v api.php nikdy
            // nebylo - graf odezvy tak dostával výchozí JSON přehled služeb
            // s kódem 200 a zůstával prázdný. metric_series vrací stejný tvar
            // ({points: [[ts, hodnota]]}), takže stačí mířit na něj.
            fetch('api.php?action=metric_series&metric=response_time&monitor_id=' + monitorId + '&period=' + period)
