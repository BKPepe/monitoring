...
| Endpoint | Access | Description |
|---|---|---|
| `action=get_settings` / `save_settings` | admin | Global settings. A key with a documented default (`bk_settings_defaults()` in db.php: the alert switches, timeouts, SLA goal, ...) reads as that default while it is unset or stored empty, never as `''`. The on/off switches (`agent_notifications_enabled`, `agent_notify_admin_only`, `daily_reminder_enabled`, `escalation_enabled`) take exactly `'1'` or `'0'`: any other value, above all `''`, makes the whole save a 400 that writes nothing and lists the keys in `invalidKeys` |
