...
| `action=link_traffic&monitor_id=&days=` | veřejné | Provoz routeru podle role linky: primární (`wan_l3_device`) vs. LTE záloha (`lte_device`) za dnes / 7 / 30 dní z denních součtů per rozhraní, plus období na záloze spárovaná z událostí `wan_lost`/`wan_restored` (otevřené období běží do teď, období započaté před oknem má `from: null`). Role bere jen z toho, co agent hlásí - bez `wan_l3_device` (agent < 0.1.3) je primární strana `null`, ne odhad podle jména |
